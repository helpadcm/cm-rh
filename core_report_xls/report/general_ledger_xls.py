...
class GeneralLegderReport(models.AbstractModel):
	...
	def generate_xlsx_report(self, workbook, data, lines): 	
		...
		sheet.write(pos, 4, _('Cuenta Presupuestaria'), format211)
		sheet.write(pos, 5, _('Analitica'), format211)
		sheet.write(pos, 6,_('Ref'), format211)
		sheet.write(pos, 7, _('Movimiento'), format211)
		sheet.write(pos, 8, _('Etiqueta'), format211)
		sheet.write(pos, 9,_('Debito'), format211)
		sheet.write(pos, 10, _('Credito'), format211)
		sheet.write(pos, 11,_('Saldo'), format211)
		...
		for account in xlines.get('Accounts',[]):
			...
			sheet.write(pos, 6, '', format211)
			sheet.write_number(pos, 9,float( account['debit']), format411)
			sheet.write_number(pos, 10, float(account['credit']), format411)
			sheet.write_number(pos, 11, float(account['balance']), format411)
			if self.env.user.has_group('base.group_multi_currency'):
				sheet.write_number(pos, 12, account.get('amount_currency') or 0, format42)
				sheet.write(pos, 13, account.get('currency_code'), format211)
			pos+=1
			...
			for line in account['move_lines']:
				if line['lid']>0:
					...
					sheet.write(pos, 4, line.get('budget'), format21)
					sheet.write(pos, 5, line.get('analytic_name'), format21)
					sheet.write(pos, 6, line['lref'], format21)
					sheet.write(pos, 7, line['move_name'], format21)
					sheet.write(pos, 8, line['lname'], format21)
					sheet.write_number(pos, 9, line['debit'], format41)
					sheet.write_number(pos, 10, line['credit'], format41)
					sheet.write_number(pos, 11, line['balance'], format41)
					if self.env.user.has_group('base.group_multi_currency'):
						sheet.write_number(pos, 12, line.get('amount_currency'), format43)
						sheet.write(pos, 13, line.get('currency_code'), format21)
				if line['lid']==0:
					#negrita
					...
					sheet.write(pos, 4, line.get('budget'), format211)
					sheet.write(pos, 5, line.get('analytic_name'), format211)
					sheet.write(pos, 6, line['lref'], format211)
					sheet.write(pos, 7, line['move_name'], format211)
					sheet.write(pos, 8, line['lname'], format211)
					if xlines.get('init_balance')==1:
						sheet.write_number(pos, 9, line['debit'], format411)
						sheet.write_number(pos, 10, line['credit'], format411)
						sheet.write_number(pos, 11, line['balance'], format411)
						if self.env.user.has_group('base.group_multi_currency'):
							sheet.write_number(pos, 12, line.get('amount_currency'), format42)
							sheet.write(pos, 13, line.get('currency_code'), format211)
				if line['lid']==-1:
					#negrita
					sheet.write(pos, 1, line['ldate'], format211)
					sheet.write(pos, 2, line['lcode'], format211)
