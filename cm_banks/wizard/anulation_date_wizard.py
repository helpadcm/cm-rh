...
class anullation_date_wizard(models.TransientModel):
	...
	date = fields.Date(string='Fecha', help=_("Fecha efectiva de anuacion"), required=True, default=lambda self: self._context['date'] if self._context and  self._context['date'] not in [False,None] else  False)
	...
	def anulate_doc(self):
		...
		if self.env.context.get('active_id'):
			...
			raise osv.except_osv(_("La operacion no pudo finalizar, intente de nuevo!"))