...
class groupGuide(models.AbstractModel):
    _name = 'report.cm_cargo_handling.guide_group_invoice'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            'child_guide': 'cm_cargo_handling.guide_group_invoice2',
            'data': self.get_data(data.get('bill_ids')),
        }

    def get_data(self, ids):
        bill_ids = self.env['cargo.bill'].search([('id','in',ids)])

        datas = []
        for order in bill_ids.mapped('order_id'):
            print_inv = False
            if order.move_id:
                print_inv = True
