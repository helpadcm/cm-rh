...
class calculateRAP(models.Model):
    ...
    name = fields.Char(string="Nombre",default="Configuracion RAP/IHSS")
    min_salary = fields.Float(string="Salario minimo")
    percentage = fields.Float(string="Porcentaje")

    ihss_amount = fields.Float(string="Monto IHSS")

    @api.model_create_multi
    def create(self, vals_list):
        rap_id = self.env['hr.settings.rap'].search([])
        if len(rap_id) == 1:
            raise ValidationError("Ya existe un registro de configuracion")
