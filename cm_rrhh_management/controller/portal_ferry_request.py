...
class ferryRequestPortal(http.Controller):
    ...
    def program_to_fly_portal(self, **kwargs):
        ...
        history_ids = request.env['rrhh.others.requests'].sudo().search([('employee_id','=',employee_id.id),('business_type','=','ferry')])
        routes_ids = request.env['cm.ferry.routes'].sudo().search([])

        actual_month = datetime.now().month
        month_requests = history_ids.filtered(lambda history: history.date.month == actual_month and history.state == 'approved')
        ...
        values = {
            ...
            'ferry_tickets': ferry_tickets,
            'history_ids': history_ids,
            'routes': routes_ids,
            'form_data': form_data  # Pasar los datos al template
        }
        return request.render("cm_rrhh_management.portal_ferry_request", values)


    @http.route('/create_beneficiary_ferry/submit', type='http', auth="user", methods=["POST"], website=True)
    def create_beneficiary_submit(self, **post):
        ...
        return request.redirect('/benefits/record_ferry_request')


    @http.route('/request_tickets_ferry/submit', type='http', auth="user", methods=["POST"], website=True)
    def request_tickets_submit(self, **post):
        ...
        qty_available = post.get("record_ferry_tickets")
        ...
        if not exit_only and not open_back:
            ...
            date_to = date_from
            if open_back:
                route_id = request.env['cm.ferry.routes'].sudo().browse(int(exit_route_id))
                if route_id:
                    exit_destination = route_id.destination
                    exit_origin = route_id.origin
                    ret_route_id = request.env['cm.ferry.routes'].sudo().search([('origin','=',exit_destination),('destination','=',exit_origin)])
                    return_route_id = ret_route_id.id

        ferry_id = request.env['cm.business.list'].sudo().search([('business_type','=','ferry')])

        vals = {
            'employee_id': employee_id.id,
            'initial_date': date_from,
            'business_id': ferry_id.id,
            'people_qty': tickets_request,
        ...
        if validated:
            if exit_only or open_back:
                if exit_only:
                    vals.update({'exit_route_id': exit_route_id, 'exit_only': True, 'exit_date': date_from})
                if open_back:
                    vals.update({'exit_route_id': exit_route_id, 'return_route_id': return_route_id, 'open_back': True, 'exit_date': date_from})
            else:
                vals.update({'exit_route_id': exit_route_id, 'return_route_id': return_route_id, 'exit_date': date_to})
            ...
            # Asignar los adjuntos creados al registro
            if attachment_ids:
                request_id.sudo().write({'attachment_ids': [(6, 0, attachment_ids)]})
