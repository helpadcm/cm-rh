...
class getMarkings(models.TransientModel):
    ...
    def connect_clocks(self):
        ...
        for clock in clocks_ids:
            # url = "http://10.1.4.56:8081/markings?ip_str=%s&port_str=%s&date=%s"%(str(clock.ip_address), str(clock.port), self.date)
            url = "http://181.115.21.90:8081/markings?ip_str=%s&port_str=%s&date=%s"%(str(clock.ip_address), str(clock.port), self.date)
