...
class HrAttendance(models.Model):
    ...
    def connect_ws(self):
        ...
        for clock in clocks_ids:
            # url = "http://10.1.4.56:8081/markings?ip_str=%s&port_str=%s&date=%s"%(str(clock.ip_address), str(clock.port), last_date)
            url = "http://181.115.21.90:8081/markings?ip_str=%s&port_str=%s&date=%s"%(str(clock.ip_address), str(clock.port), last_date)
